...
namespace AdventureText.MathParsing
{
    ...
    public static class Parser
    {
        ...
        /// fractions. False by default.
        /// </summary>
        public static bool OptUseFractions;

        /// <summary>
        /// Null identifiers will be replaced with this token for evaluation,
        /// if specified. Else, an error will be thrown. Null by default.
        /// </summary>
        public static Token OptUnknownDefault;
        #endregion
        ...
        /// </summary>
        public static readonly LiteralId VarTrue;

        /// <summary>
        /// Represents a false boolean value.
        /// </summary>
        public static readonly LiteralId VarFalse;
        ...
        static Parser()
        {
            ...
            VarTrue = new LiteralId("true", true);
            VarFalse = new LiteralId("false", false);
            ...
            //Sets the list of loaded tokens.
            ResetTokens();
        }
        #endregion
        ...
        public static void AddIdentifier(LiteralId token)
        {
            ...
            if (token.Value is decimal decVal)
            {
                tokens.Add(new LiteralId(
                    token.StrForm.ToLower(), decVal));
            }
            else if (token.Value is bool boolVal)
            {
                tokens.Add(new LiteralId(
                    token.StrForm.ToLower(), boolVal));
            }
        ...
        /// An expression composed of numbers, bools, included operators
        /// and functions.
        /// </param>
        ...
        /// <summary>
        /// Parses a pre-tokenized expression. Invalid tokenization may
        /// result in unanticipated errors.
        /// </summary>
        /// <param name="expression">
        /// A list of tokens representing numbers, bools, included operators
        /// and functions.
        /// </param>
        public static string Eval(List<Token> tokensList)
        {
            //Substitutes values for identifiers.
            for (int i = 0; i < tokensList.Count; i++)
            {
                if (tokensList[i] is LiteralId tokId)
                {
                    //Inserts any known values for a token.
                    if (tokId.Value != null)
                    {
                        if (tokId.Value is decimal)
                        {
                            tokensList[i] = new LiteralNum((decimal)tokId.Value);
                        }
                        else if (tokId.Value is bool)
                        {
                            tokensList[i] = new LiteralBool((bool)tokId.Value);
                        }
                    }

                    //Replaces unknown identifiers with a value or fails.
                    else if (OptUnknownDefault != null)
                    {
                        tokensList[i] = OptUnknownDefault;
                    }
                    else
                    {
                        throw new ParsingException("The identifier '" +
        ...
        public static List<Token> Tokenize(string expression)
        {
            ...
            for (int i = 0; i < expression.Length; i++)
            {
                ...
                if (longestMatch?.StrForm.Length > token.Length)
                {
                ...
                else
                {
                    ...
                    {
                        tokensList.Add(new LiteralId(token[0].ToString()));
                        i -= token.Length - 1;
                        token = "";
                    }
                    else
            ...
            if (OptUseImplicitMult)
            {
                return tokensList;
            }

            //When implicit multiplication is off, tokens that would have
            //to be implicitly multiplied to compute are combined instead.
            //If the resulting token exists, uses it. Else, adds as unknown
            //or throws an error.
            List<Token> combinedTokens = new List<Token>();
            string unknownTokenName = "";

            //Combines contiguous unknowns if implicit multiplication is off.
            //Sets tokens to be subtraction rather than negation by default.
            for (int i = 0; i < tokensList.Count; i++)
            {
                //Break and add while looking ahead.
                if (i == tokensList.Count - 1 ||
                    tokensList[i + 1] is Operator ||
                    tokensList[i + 1] is Symbol)
                {
                    if (unknownTokenName != "")
                    {
                        unknownTokenName += tokensList[i].StrForm;

                        if (OptIncludeUnknowns)
                        {
                            combinedTokens.Add(new LiteralId(unknownTokenName));
                            unknownTokenName = "";
                            continue;
                        }
                        else
                ...
                //Append.
                else if (!(tokensList[i] is Operator ||
                    tokensList[i] is Symbol))
                {
                    if (unknownTokenName != "" ||
                        !(tokensList[i] is LiteralNum))
                    {
                        unknownTokenName += tokensList[i].StrForm;
                        continue;
                    }
                ...
                if (tokensList[i] == OpNeg)
                {
                    combinedTokens.Add(OpSub);
                }
                else
                {
                    combinedTokens.Add(tokensList[i]);
                }
            }
